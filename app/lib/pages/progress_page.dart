// Modified for Sendy: transfer overview driven by session status and byte counters.
import 'dart:async';
...
class _ProgressPageState extends State<ProgressPage> with Refena {
  ...
  Widget build(BuildContext context) {
    ...
    final animations = ref.watch(animationProvider) && !MediaQuery.disableAnimationsOf(context);
        ...
        body: Stack(
          children: [
            ListView.builder(
              ...
              itemCount: _files.length + 3,
              itemBuilder: (context, index) {
                if (index == 0) {
                  return SendyTransferSummary(
                    key: ValueKey(widget.sessionId),
                    phase: sendyTransferPhase(status),
                    statusLabel: status.getLabel(remainingTime: _remainingTime ?? '-'),
                    peer: receiveSession?.senderAlias ?? sendSession?.target.alias ?? '',
                    details: _files.isEmpty ? '' : '${currBytes.asReadableFileSize} / ${_totalBytes.asReadableFileSize}',
                    progress: status == SessionStatus.waiting || _totalBytes <= 0 || _files.isEmpty ? null : currBytes / _totalBytes,
                    animate: animations,
                  );
                }
                if (index == 1) {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          TweenAnimationBuilder(
                            ...
                            duration: animations ? const Duration(milliseconds: 200) : Duration.zero,
