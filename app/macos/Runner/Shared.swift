// Modified for Sendy: support unsigned builds without the upstream Apple app group.
import Foundation
import Defaults

let teamIdentifierPrefix = Bundle.main.infoDictionary?["AppIdentifierPrefix"] as? String ?? ""
let hasSigningTeam = !teamIdentifierPrefix.isEmpty && !teamIdentifierPrefix.contains("$(")
let sharedDefaults = hasSigningTeam
    ? (UserDefaults(suiteName: "\(teamIdentifierPrefix)sendy.shared_group") ?? .standard)
    : .standard
